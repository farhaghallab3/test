...
class WorkerAppBar extends StatelessWidget implements PreferredSizeWidget {
  final GlobalKey<ScaffoldState> scaffoldKeyw;
  final bool showSearchBox; // New parameter to control search box visibility

  const WorkerAppBar({
    Key? key,
    required this.scaffoldKeyw,
    this.showSearchBox = false,
    required GlobalKey<ScaffoldState> scaffoldKey, // Default value is false
  }) : super(key: key);
    ...
    return AppBar(
      ...
      title: Row(
        children: [
          IconButton(
            onPressed: () {
              scaffoldKeyw.currentState?.openDrawer(); // Open the drawer
            },
            icon: const Icon(
              Icons.menu,
              color: Colors.white,
